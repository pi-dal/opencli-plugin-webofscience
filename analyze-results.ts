...
import { basicSearchUrl, normalizeDatabase } from './src/lib/shared';
...
const FACET_CHOICES = Object.keys(FACET_LABELS);
...
cli({
  ...
  func: async (page, kwargs) => {
    ...
    // Extract facet data from the Refine Results sidebar
    ...
    return facetData.slice(0, limit).map((item: { value: string; count: number }, index: number) => ({
      rank: index + 1,
      value: item.value,
      count: item.count,
    }));
